...
def RefreshTable(phase):
    ...
    with open(str(phase) + 'matches.txt', 'r', encoding='utf-8') as file:
        for line in file:
            ...
            if match[2] > match[3]: #ПЗР
                content[t1Row][2] = str(int(content[t1Row][2]) + 1)
                content[t2Row][3] = str(int(content[t2Row][3]) + 1)
                
                content[t1Row][8] = str(int(content[t1Row][8]) + 3) #Toчки
            elif match[3] > match[2]:
                content[t1Row][3] = str(int(content[t1Row][3]) + 1)
                content[t2Row][2] = str(int(content[t2Row][2]) + 1)

                content[t2Row][8] = str(int(content[t2Row][8]) + 3)#Toчки
            else:
                content[t1Row][4] = str(int(content[t1Row][4]) + 1)
                content[t2Row][4] = str(int(content[t2Row][4]) + 1)

                content[t1Row][8] = str(int(content[t1Row][8]) + 1)#Toчки
                content[t2Row][8] = str(int(content[t2Row][8]) + 1)#Toчки

            content[t1Row][5] = str(int(content[t1Row][5]) + int(match[2]))
            content[t2Row][5] = str(int(content[t2Row][5]) + int(match[3]))#ОГ

            content[t1Row][6] = str(int(content[t1Row][6]) + int(match[3]))
            content[t2Row][6] = str(int(content[t2Row][6]) + int(match[2]))#ДГ

            content[t1Row][7] = str(int(content[t1Row][5]) - int(content[t1Row][6]))
            content[t2Row][7] = str(int(content[t2Row][5]) - int(content[t2Row][6]))#Раз

            content[t1Row][9] = str(round(int(content[t1Row][8]) / int(content[t1Row][1]), 3))
